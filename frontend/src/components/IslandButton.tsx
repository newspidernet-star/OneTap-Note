...
export default function IslandButton({
  ...
  const showHeroCard = isLarge && (
    (status === "idle" && !hasMedia) ||
    status === "error" ||
    isPipeline ||
    (status === "done" && !hasMedia)
  );
  const showMediaPreview = hasMedia && (status === "idle" || status === "done");
          ...
          >
            ...
            <div className="flex items-center justify-between gap-2">
              <div className="flex items-center gap-1.5 text-xs text-emerald-500">
                <Check className="w-3.5 h-3.5" /> {status === "done" ? "总结已生成" : "媒体已就绪"}
              </div>
              <button
                onClick={status === "done" ? onRegenerate : onGenerate}
                disabled={status !== "done" && !generable}
                className="flex items-center gap-1 text-xs text-muted-foreground hover:text-foreground transition-colors px-2 py-1 rounded-md hover:bg-foreground/5 disabled:opacity-40"
              >
                <RefreshCw className="w-3 h-3" /> {status === "done" ? "重新生成笔记" : "生成知识笔记"}
              </button>
            </div>
          </motion.div>
        )}

        {!showHeroCard && !showMediaPreview && status !== "done" && (
          <motion.div
            key="compact"
            layout
            initial={{ opacity: 0, scale: 0.96 }}
            ...
            {status === "error" ? (
              <div className="w-full rounded-xl border border-red-500/20 bg-red-500/5 flex flex-col items-center gap-2.5 py-3">
                <div className="flex items-center gap-1.5 text-xs text-red-500">
                  <AlertCircle className="w-4 h-4" /> {errorMessage || "鐢熸垚澶辫触"}
                </div>
                <button onClick={onGenerate} disabled={!generable}
                  ...
                  <RefreshCw className="w-3.5 h-3.5" /> 閲嶈瘯
                </button>
              </div>
            ) : (
              <button
              ...
              >
                <span>{status === "done" && !hasMedia ? "閲嶆柊鐢熸垚绗旇" : "鐢熸垚鐭ヨ瘑绗旇"}</span>
