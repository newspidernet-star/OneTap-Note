...
> 把短视频变成个人知识库素材：粘贴链接或上传视频/音频/图片，自动转写 + OCR + AI 总结，每个结论可追溯到具体画面或语音，导出 Obsidian 笔记。

[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)
[![Release](https://img.shields.io/badge/Release-v0.1.0-blue)](https://github.com/newspidernet-star/smart-scribe/releases/tag/v0.1.0)
...
Smart Scribe 就是为这个场景做的：已有素材 → 证据化整理 → AI 总结 → Obsidian 知识卡片。
...
Smart Scribe 把视频抽成关键画面做 OCR，也把音频转成文字。AI 总结时画面证据和语音证据一起参与，术语以画面 OCR 为准。
...
AI 生成总结时带着这些引用，看到一个结论时可以回头找到它来自哪一段语音、哪一张画面。
...
一个视频不够完整时，可以继续补一张截图、补一段音频、补另一个视频。追加后自动重新生成总结，不用手动点"重新生成"。

### 6. 导出成 Obsidian 笔记

导出的 Markdown 包含 YAML frontmatter、AI 摘要、核心要点和证据引用、来源链接、证据索引、可折叠的详细原文——一份干净的知识卡片底稿。

## 快速开始

### 方法一：下载 Release（普通用户推荐）

1. 到 [Releases 页面](https://github.com/newspidernet-star/smart-scribe/releases/tag/v0.1.0) 下载 `Smart-Scribe-Windows.zip`
2. 解压
...
AI 总结基于证据块生成，而不是只看一段转写文本。画面 OCR 和语音 ASR 一起纳入判断。如果画面文字和语音转写冲突，通常优先相信画面 OCR——因为很多专有名词、项目名、代码片段，画面上的文字比语音转写更可靠。
