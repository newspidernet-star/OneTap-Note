...
更底层一点说，视频最大的问题不是信息少，而是信息被锁在时间轴里。

一个观点可能藏在 53 分钟视频的第 37 分钟；一个方法可能只出现了 20 秒；一个以后会启发你的灵感，刷过去之后就很难再找回来。视频不像文本那样天然可以搜索，也很难直接进入个人知识库参与后续思考。

One Tap Note 想做的事，就是把这些原本只能“看一遍”的视频和媒体素材，转化成可以搜索、可以引用、可以整理进 Obsidian 的知识资产。等你以后写文章、准备面试、复习知识点、做项目决策时，它不再只是一个收藏夹里的链接，而是能被重新调用的个人积累。
...
有时留下一个视频，并不是因为它讲了一套完整知识。可能只是因为它的音效很好、转场很顺、某句话很有意思，或者以后做类似项目时想回来参考。

这种内容如果也必须先下载、转写、等待 AI 总结，记录成本反而比信息本身还重。One Tap Note 因此把链接输入分成两条路径：

| 模式 | 适合什么内容 | 系统会做什么 |
| --- | --- | --- |
| **快速收下** | 灵感、案例、音效、画面、以后可能参考的内容 | 立即保存原链接，可选留一句话；不下载、不转写、不调用 AI |
| **生成知识笔记** | 需要理解、搜索、引用和长期复用的内容 | 下载素材、语音转写、整理结构、生成知识笔记 |

“快速收下”保存的不是一篇被强行扩写的摘要，而是一个**记忆锚点**。最少只需要一个链接；如果当下有想法，可以补上“为什么留下”和“以后可能用在哪里”：
...
这仍然是 One Tap 的同一个内核：先用最低成本把值得留下的东西固定住，是否深度处理可以以后再决定。
...
5. 上传本地素材，或者粘贴视频链接
6. 轻量灵感点“快速收下”，需要完整消化的内容点“生成知识笔记”
7. 将轻笔记或知识笔记复制到自己的 Obsidian 里
...
下载链接素材后，One Tap Note 会保留原始来源。你可以在播放器下方直接复制原链接，或者打开原视频页面，回到原站继续看评论、作者主页和上下文。

如果需要把视频本身放进笔记系统，也可以直接下载当前视频文件。这样笔记不只留下文字结论，也能在需要时保留原始媒体素材。

### 快速收下

粘贴一个链接后，可以不进入下载和 AI 处理流程，直接点“快速收下”。系统会立即创建一条轻记录并保存原始来源。

保存后可以补一句自己的判断，例如“这个视频的音效和转场很好，以后做产品介绍视频时参考”。备注会自动保存，也可以直接复制成一份干净的 Markdown 轻笔记。如果后来发现它值得认真消化，可以在同一条记录里点“整理成知识笔记”，再进入完整的下载、转写和生成流程。

这条路径适合那些“现在只想记住，暂时不值得深度处理”的内容，避免为了保存一个灵感而等待完整转写。
...
AI 可以整理视频内容，但一条信息为什么对你有用，往往只有你自己知道。

所以现在每条知识笔记都可以写一段“我的随手记”。它不经过 AI 改写，也不会被模型重新总结，而是作为你自己的判断原样保存：

- 这条内容可以用到哪里。
- 哪句话值得以后引用。
- 它和你正在做的项目、文章、学习任务有什么关系。
- 你看完之后临时冒出来的想法。

复制知识笔记时，这部分会自动合并到 Markdown 里，成为笔记中的 `我的随手记` 小节。这样最后进入 Obsidian 的不只是视频摘要，也有你自己的理解和使用场景。
...
这次改动对 One Tap Note 来说不是单纯加几个按钮，而是从“能生成笔记”往“真的适合长期使用”走了一步。

过去的主流程是：

```text
链接 / 素材 -> 转写 -> 生成知识笔记 -> 复制到 Obsidian
```

现在补上了三个关键环节：

```text
快速收下 + 原始来源可回溯 + 用户自己的随手判断
```

具体来说：

- **快速收下**：只保存链接和可选的一句话，不下载、不转写、不调用 AI，让灵感先以最低成本留下来。
- **原链接保留**：处理完链接素材后，仍然可以复制原链接、打开原视频，回到原站查看评论和上下文。
- **视频可下载**：本地已经下载过的视频，不再只能留在应用里播放，也可以导出为文件，方便和笔记一起保存。
- **我的随手记**：用户可以写自己的用途、灵感和判断，并在复制 Markdown 时自动并入知识笔记。

这一步让 One Tap Note 不只是“帮我总结一下视频”，而是更接近一个完整的信息内化流程：轻内容可以先留下锚点，重内容可以被认真整理，来源可以被找回，用户自己的判断也能保留下来。
...
5. 生成知识笔记。
6. 在“我的随手记”里补一句自己的用途或想法。
7. 需要时打开原链接回到原视频，或者下载视频文件。
8. 复制 Markdown，放进 Obsidian。
