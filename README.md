...
自托管的 AI 媒体总结工作台。上传视频 / 音频 / 图片或**粘贴链接** → 自动转写 + OCR + AI 总结，**每一个结论都能追溯到具体的 PPT 页或语音时间点**。

定位是"专业工作台"而不是"网页表单"——Premiere / DaVinci 那种暗色工作站美学，按一次按钮走完流水线，结果可引用、可追溯。

## ✨ 亮点：直接粘贴链接，不用先下载

不用手动下载视频再上传——**粘贴一个链接，Smart Scribe 自己抓回来处理**。市面上的 AI 总结工具几乎都要求你先下载好文件再上传，Smart Scribe 把这一步省了。

支持的平台：

| 平台 | 抓取方式 | 需要 cookie？ |
|------|---------|--------------|
| **抖音**（视频 / 图文） | 自有实现：解析移动端分享页 SSR 数据，去水印下载 | ❌ 免 cookie |
| **Bilibili**（含 `b23.tv` 短链） | 自有实现：调 B 站公开 API 取播放地址 | ❌ 免 cookie |
| **YouTube** | yt-dlp | 可选（会员内容需要） |
| **任意直链**（`.mp4` / `.mp3` / `.jpg` ...） | 直接下载 | ❌ |
| **其他 yt-dlp 支持的站点** | yt-dlp | 可选 |

> 抖音和 B 站是**专门写的免 cookie 抓取**（`backend/app/services/downloader.py`），不依赖 yt-dlp，也不需要你登录任何账号——粘分享文本里的链接就行。

## ✨ 亮点：多源混合，一次喂完

同一会话里可以**任意组合**多种媒体：本地文件 + 链接抓取，视频 + 音频 + 图片，想加几个加几个。Smart Scribe 按类型分别处理，再统一对齐成证据块，AI 总结时一起引用。

| 类型 | 怎么处理 | 产出证据块 |
|------|---------|----------|
| 视频 | ffmpeg 抽帧 → PaddleOCR 识别 PPT / 字幕 | P 图像块（带时间戳 + 页号）|
| 音频 | 阿里云 Fun-ASR 转写 + 说话人分离 | S 语音块（带时间戳 + 说话人）|
| 图片 | PaddleOCR 识别文字 | P 图像块（带页号）|

**这意味着什么**：一段课堂录像 + 老师发的 PPT 图片 + 课堂录音 → 喂进同一个会话 → AI 总结能同时引用"第 3 页 PPT 上的公式"和"23:15 同学说的话"，互相印证。市面上的工具大多只处理单一类型（要么只转写音频，要么只总结文档），Smart Scribe 是**真正意义上的混合媒体总结**。

## ✨ 亮点：画面文字优先，不受语音转写错误影响

语音转写（ASR）常有同音字 / 术语错误——把"大江大河"转成"大寻劝"、把项目名听错。Smart Scribe 的 AI 总结**以画面 OCR 文字为权威来源**，ASR 只做补充：

- **OCR 放在 prompt 最前面**，AI 先看到准确版本，ASR 放后面标注为"可能有同音字错误"
- **纠错原文**：AI 根据 OCR 修正 ASR 里的同音字和术语错误（如 ASR"大寻劝"→ OCR"大江大河"→ 纠正）
- **核心要点 / 摘要的术语名词**优先用 OCR 显示的原文，不以语音转写为准
- **会话标题**优先用 OCR 文字生成（项目介绍类视频标题不再被 ASR 错翻带偏）

当画面和语音冲突时，AI 会以画面为准——这在你处理项目介绍、技术教程这类"画面有准确文字"的视频时效果尤其明显。

## ✨ 亮点：增量幻灯片重建，不是每帧傻 OCR

传统方案是"抽 300 帧 → 全部 OCR → 300 个结果"——又慢又重复。Smart Scribe 用**三阶段增量重建**模拟"人看 PPT 翻页"的过程：

| 阶段 | 做什么 | 效果 |
|------|--------|------|
| **1. 场景分组** | 按帧差异度把视频分成"幻灯片组"——同一张 PPT 上加文字不触发分组（累积态留在同一组），翻页才分新组 | 300 帧 → 30-50 组 |
| **2. 字幕检测** | 每组比较首尾帧底部区域——位置固定但内容在变 = 字幕，排除掉（和 ASR 重复是噪音） | 自动识别有无字幕 |
| **3. 取累积最完整帧** | 每组只取**文字量最多**的那帧（5s "AI Agent" → 15s "AI Agent + 定义 + 工作流 + MCP" → 取 15s 那帧）。高重合度的帧合并保留文字更多的 | 30-50 组 → 最多 30 帧 |

**跨帧文字去重**：OCR 完所有帧后，比较相邻帧文字重合度 >85% 的只保留文字更多的那个——10 个"从空 PPT 到填满"的帧只留 1 个最完整的。

结果：**30 帧的 OCR 调用砍到 10-15 个有效块**，AI 总结读的输入更少更干净，证据时间线不再被重复内容刷屏。

## 功能

- 上传本地媒体文件（**多文件、多类型混合**：视频 + 音频 + 图片任意组合），或**粘贴视频链接**（抖音 / B 站 / YouTube / 任意直链）
- 视频自动抽帧做 **OCR**（识别 PPT / 字幕），音频做 **语音转写**（说话人分离），图片做 **OCR**
- 把转写片段和 OCR 片段对齐成"证据块"（S=Speech / P=Picture）
- 一键生成带引用的 **AI 总结**（核心要点 / 摘要 / 纠错原文），**每个结论都标注来源证据块**
- **点击引用标签 → 视频自动跳转到对应时间点 + 高亮该证据块**：AI 说"23:15 同学提问"，点一下就跳到视频那个时刻，同时时间线里对应块高亮，原文和总结双向追溯
- AI 自动校验引用真实性，列出**未被引用的片段**，避免遗漏
